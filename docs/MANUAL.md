...
| Metric model             | T-Student distribution with muliple priors       | metric observations                | Exact match, Accuracy, Recall, UAS(sentencelevel), LAS(sentencelevel), running time. energy usage, L2 error                                                                                                                                                                                                                                                                                                                | t-test                               | bootstrap / permutation                  | 1,2,4       | In this model (unlike frequentist t-test) outliers don't need to be discarded manually to realize the strict normality assumption. The observations are assumed to follow t-distirbution with unknown normality parameter |
| Count model              | Negative Binomial distribution with Normal prior | counts                             | The count of certain patterns an algorithm could find in a big pool, in a fixed amount of time. Notice that you cannot convert this into a ratio form, since there is no well-defined denominator. For example, lets say there are an infinite number of questions and you want to see how many of them are answered correctly by a particular QA systems, in one minute (the system is allowed to skip the questions too) |                                      | bootstrap / permutation                  | 2,5         |                                                                                                                                                                                                                           |
| Ordinal model            | Normal distribution with parameterized tresholds | ordinals                           | Collection of objects/labels arranged in a certain ordering, not necessarily with a metric distance between them; for example [https://www.aclweb.org/anthology/S16-1001.pdf sentiment labels], product review categories, grammaticality of sentences                                                                                                                                                                     |                                      | bootstrap / permutation                  | 2           |                                                                                                                                                                                                                           |
| Contingency observations | TBA (not implemented yet)                        | Contengency table/confusion matrix |                                                                                                                                                                                                                                                                                                                                                                                                                            |                                      | McNemar's test, bootstrap / permutation  | 2           |                                                                                                                                                                                                                           |
| Generalized metric model | TBA (not implemented yet)                        | metric observations                |                                                                                                                                                                                                                                                                                                                                                                                                                            |                                      | bootstrap / permutation                  | 2           |                                                                                                                                                                                                                           |

 The assumptions used in the above table: 
 
 - Assumption 1: assuming that the observations are distributed as a t-student  (a normal distribution with potentially longer tales).
 ...
 - Assumption 4: Could be used when (1) granularity (denomenator) is high enough to assume the variable is continuous Or (2) when the variable is inherently continous
 - Assumption 5: the observations expected to be follow a Negative-Binomail / Poisson distribution. 
